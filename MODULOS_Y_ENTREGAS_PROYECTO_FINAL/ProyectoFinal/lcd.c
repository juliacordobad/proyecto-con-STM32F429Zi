...
void LCD (void *argument) {
  ...
  while (1) {
     // Insert thread code here...
		...
		if(mensajesColaRecibido.modo == 1){ //MODO REPOSO
			sprintf(linea1, "     SBM 2024");
			for(int i = 0; linea1[i] != '\0'; i++){
				symbolToLocalBuffer(1, linea1[i]);
			}
			sprintf(linea2, "      %02d:%02d:%02d", mensajesColaRecibido.horas, mensajesColaRecibido.minutos, mensajesColaRecibido.segundos);
			for(int i = 0; linea2[i] != '\0'; i++){
				symbolToLocalBuffer(2, linea2[i]);
			}
			
		}else if (mensajesColaRecibido.modo == 2){
			sprintf(linea1, "    ACTIVO--T:%.1f*", mensajesColaRecibido.temperatura);
			for(int i = 0; linea1[i] != '\0'; i++){
				symbolToLocalBuffer(1, linea1[i]);
			}
			sprintf(linea2, "  X:%.2f Y:%.2f Z:%.2f", mensajesColaRecibido.aceleracionX, mensajesColaRecibido.aceleracionY, mensajesColaRecibido.aceleracionZ);
			for(int i = 0; linea2[i] != '\0'; i++){
				symbolToLocalBuffer(2, linea2[i]);
			}
			...
			if(mensajesColaRecibido.tipoDepuracion == 0){
					sprintf(linea2, "   H: %02d:%02d:%02d", mensajesColaRecibido.horas, mensajesColaRecibido.minutos, mensajesColaRecibido.segundos);
				for(int i = 0; linea2[i] != '\0'; i++){
					symbolToLocalBuffer(2, linea2[i]);
				}
			}
			if(mensajesColaRecibido.tipoDepuracion == 1){
				sprintf(linea2, "Ax:%.2f Ay:%.2f Az:%.2f", mensajesColaRecibido.refAx, mensajesColaRecibido.refAy, mensajesColaRecibido.refAz);
				for(int i = 0; linea2[i] != '\0'; i++){
					symbolToLocalBuffer(2, linea2[i]);
				}
			}
